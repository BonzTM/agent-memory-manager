...
function updateValuesYaml(valuesYamlPath, releaseTag) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    // Detect the top-level image: block
    if (/^image:\s*$/.test(line)) {
      inImageBlock = true;
      continue;
    }

    // Exit image block when we hit another top-level key
    if (inImageBlock && /^[A-Za-z]/.test(line)) {
      inImageBlock = false;
    }

    // Update the tag field inside the image block
    if (inImageBlock && /^  tag:\s*.*$/.test(line)) {
  ...
  if (!tagUpdated) {
    fail(`Failed to update image tag field in ${valuesYamlPath}.`);
  }
...
function main() {
  ...
  console.log(
    `Prepared Helm chart release metadata at ${chartDir} (version=${chartVersion}, appVersion=${releaseTag}, imageTag=${releaseTag}).`,
  );
}
