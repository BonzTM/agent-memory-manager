...
- Summaries carry a `depth` field (0 = leaf/session/entity_brief, 1 = topic/condensed, 2+ = higher condensed) enabling efficient DAG queries by level.
- Entity briefs (`kind: entity_brief`) synthesize all knowledge about an entity into a coherent narrative, generated by the `build_entity_briefs` job for entities with 3+ linked memories.
