...
- **Optional Hermes curated-memory parity in the repo-shipped plugin.** `examples/hermes-agent/amm-memory` can now mirror successful Hermes `memory` tool writes into AMM durable memories via `post_tool_call`, with env-driven scope/type configuration plus a local AMM-ID map and retry queue for update/delete targeting.
